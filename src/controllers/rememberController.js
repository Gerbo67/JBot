            ...
            if (title.length < 100) {
                if (body.length < 200) {
                    ...
                    if (resultRemember > 0) {
                        ...
                        await interaction.reply({content: 'No se ha guardado, favor de contactar con el administrador', ephemeral: true});
                    }
                } else {
                    await interaction.reply({content: 'El cuerpo no puede tener mas de 199 caracteres', ephemeral: true});
                }
            } else {
                await interaction.reply({content: 'El titulo no puede tener mas de 99 caracteres', ephemeral: true});
            }
        } else {
...
async function deleteRemember(interaction) {
    ...
    if (result > 0) {
        ...
        await interaction.reply({content: 'No pude ser eliminado o no corresponde a un id existente', ephemeral: true});
    }
}
